        ...
        return (
          <TouchableOpacity key={item.value} activeOpacity={0.7}
            ...
            <Text style={{ fontSize: 13, fontWeight: '600',
              color: sel ? t.accentText : t.text2 }}>{item.label}</Text>
          </TouchableOpacity>
        );
      })}
    </ScrollView>
...
export default function MaintenanceEditScreen() {
  ...
  /* Pre-fill form once */
  useEffect(() => {
    if (!job || hasInitialized.current) return;
    hasInitialized.current = true;
    setTitre(job.titre ?? '');
    setDescription(job.description ?? '');
    setNature(job.nature ?? 'reactif');
    setTypeTravail(job.type_travail ?? 'autre');
    setPriorite(job.priorite ?? 'normale');
    setAppartementId(job.appartement ?? null);
    setResidenceId(job.residence ?? null);
    setAssigneA(job.assigne_a ?? null);
    setDatePrevue(job.date_prevue ? job.date_prevue.split('T')[0] : '');
    setRecurrence(job.recurrence ?? 'aucune');
  }, [job]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ['maintenance-detail', id] });
      router.back();
    } catch (err: any) {
      const d = err?.response?.data;
      setError(
        typeof d === 'string' ? d :
        d?.detail ?? d?.titre?.[0] ?? d?.non_field_errors?.[0] ?? 'Erreur lors de la sauvegarde.'
      );
    } finally {
      setLoading(false);
  ...
  if (jobLoading) {
    return (
      <View style={{ flex: 1, backgroundColor: t.bg }}>
        <DetailHeader t={t} title="Modifier" onBack={() => router.back()} />
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.bg }}>
      <DetailHeader t={t} title="Modifier le signalement" onBack={() => router.back()} />
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, gap: 16 }}
        ...
        <View>
          ...
          <TextInput style={[inputStyle, { height: 88, textAlignVertical: 'top', paddingTop: 11 }]}
        ...
        <TouchableOpacity onPress={handleSubmit} disabled={!canSubmit || loading}
          ...
          {loading
            ...
            : <Text style={{ fontWeight: '800', fontSize: 15, color: '#fff',
                letterSpacing: -0.2 }}>Enregistrer les modifications</Text>
          }
        </TouchableOpacity>
      </View>
    </View>
